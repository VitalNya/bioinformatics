...
#C:/Users/vitalnya/
#C:/Users/vital/
imp.all.rpm2 = read.table( "C:/Users/vital/Box/AndersonLab_Cybox/Vital_Nyabashi/timepoint.rpm.txt")
...
imp.11.rpm$B_mean <- rowMeans(imp.11.rpm[,4:5])
...
```{r}
zein<- read.table("//las-dfs-01.las.iastate.edu/lss/research/sna-lab/kmh/projects/NAM/all_zein_mat_pref_synts.txt")
names(zein)<- zein[1,]
zein<-zein[-1,]
...
data_set<-merged_df[!grepl("NaN",merged_df$maternal_preference),]
...
data_set_Function<- function(gene){
  ...
  ggplot(data_set, aes(x = contrast, y = maternal_preference, color = exp)) + 
    ...
    ylim(0, 1) + theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=.5)) + 
    ggtitle(gene)
}
...
x <- data_set_Function("Zm00004b028826")
